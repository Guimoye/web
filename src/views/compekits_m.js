...
export default class CompekitsM extends Component {
    ...
    save = (e) => {
        ...
        Api.post('/compekits/create', this.state.item, (rsp) => {
    ...
    remove = () => {
        if (window.confirm('¿Seguro que quieres borrar?')) {
            ...
            Api.get('/compekits/remove', {id: this.state.item.id}, (rsp) => {
                if (rsp.ok) {
            ...
            <Modal isOpen={this.state.modal} className={this.props.className} backdrop="static" keyboard={false}>
                <fieldset disabled={this.state.loading}>
                    <ModalHeader toggle={this.toggle}>
                        {this.state.item.id > 0 ? 'Editar' : 'Agregar'} método
                    </ModalHeader>
